...
@test "zellij-clients requires PID argument" {
    run "$WKFLW_NTFY_TEST_DIR/.local/lib/wkflw-ntfy/macos/wkflw-ntfy-zellij-clients.py"
    [ "$status" -eq 1 ]
...
@test "zellij-clients rejects invalid PID" {
    run "$WKFLW_NTFY_TEST_DIR/.local/lib/wkflw-ntfy/macos/wkflw-ntfy-zellij-clients.py" "not-a-number"
    [ "$status" -eq 1 ]
...
@test "zellij-clients handles non-existent PID" {
    run "$WKFLW_NTFY_TEST_DIR/.local/lib/wkflw-ntfy/macos/wkflw-ntfy-zellij-clients.py" 999999
    [ "$status" -eq 1 ]
    # Output should be valid JSON with error
    echo "$output" | jq -e '.error' > /dev/null
...
@test "zellij-clients outputs valid JSON structure" {
    ...
    run "$WKFLW_NTFY_TEST_DIR/.local/lib/wkflw-ntfy/macos/wkflw-ntfy-zellij-clients.py" 1
...
@test "zellij-clients finds server if one exists" {
    ...
    run "$WKFLW_NTFY_TEST_DIR/.local/lib/wkflw-ntfy/macos/wkflw-ntfy-zellij-clients.py" "$server_pid"
    [ "$status" -eq 0 ]

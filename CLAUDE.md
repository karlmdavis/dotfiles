...
- `~/.claude/settings.json` - Global preferences, plugins, and comprehensive pre-approved permissions.
    Generated by `private_dot_claude/modify_settings.json.tmpl`, which owns every key it declares
    ("chezmoi wins") but carries a short list of runtime-owned keys (e.g. `model`, written by
    `/model`) through from the live file, and compares semantically so Claude Code's key
    reordering never shows as a diff.
    If `chezmoi status` starts showing this file as perpetually modified, Claude Code has begun
    persisting a new key: add it to `RUNTIME_KEYS` in that script.

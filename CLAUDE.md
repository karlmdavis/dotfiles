...
The top-level `README.md`'s "Repository Map" section is the source of truth for where things
  live: an annotated `ls`-tree of repo + chezmoi meta, the chezmoi source-state prefix grammar
  (`dot_`, `private_dot_`, `executable_`, `.tmpl`), and a categorized "Apps / Configs Managed"
  list.
...
- `~/.claude/settings.json` - Global preferences, plugins, and comprehensive pre-approved permissions.
    Generated by `private_dot_claude/modify_settings.json.tmpl`, which owns the whole file
    ("chezmoi wins"): keys it declares are enforced, keys it doesn't declare are stripped, and it
    compares semantically so Claude Code's key reordering never shows as a diff.
    The one exception is `RUNTIME_KEYS` in that script (currently just `model`): keys whose
    value is legitimately chosen inside Claude Code per machine (e.g. via `/model`) and carried
    through from the live file rather than stripped.
    If `chezmoi status` starts showing this file as perpetually modified, Claude Code has begun
    persisting a new key; the default response is to declare it in the template with the value
    you want everywhere, and only add it to `RUNTIME_KEYS` if it genuinely should vary per
    machine and be set from inside Claude Code.
- `~/.claude/settings.local.json` - Machine-specific overrides (minimal, for truly local settings).
    ...
    it is merged into `settings.json` at apply time with objects recursing, lists unioned onto the
    committed ones, and scalars replaced.

...
- `~/.claude/settings.json` - Global preferences, plugins, and comprehensive pre-approved permissions.
    ...
    is stripped, and it compares semantically so Claude Code's key reordering never shows as a
    diff.
    ...
    If `chezmoi status` shows this file as perpetually modified even though `chezmoi diff` is
    only reordering plus one new key, Claude Code has begun persisting that key.
